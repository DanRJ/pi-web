...
The composer remains the existing controller-owned CodeMirror editor. Draft storage, slash and file completions, paste/drop/file capture, attachment delivery, model/thinking selection, shell mode, steer/follow-up behavior, send, and Stop are preserved. Classic and PI WEB retain their legacy composer structure: Attach remains in the editor, followed by compact status, Send/Queue, conditional Steer, and Stop. Modernist alone uses its grouped action row: Attach, Model, and Thinking context controls precede conditional Steer, Stop, and the rightmost Send/Queue primary action. The inactive theme template is `display: none`, so duplicate controls are neither focusable nor exposed to assistive technology. Streaming status updates avoid remounting the editor when their rendered model and thinking fields have not changed. On narrow screens, a yielding model label is the only action-row content allowed to truncate; attach, send, steer, and Stop controls retain their touch targets and labels. At extra-narrow container widths the context and execution groups deliberately become two rows rather than overflowing.
