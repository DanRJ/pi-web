...
The initial browser component renders inline cards for `select`, `confirm`, `input`, and `editor`, plus transient notifications. When a model calls `ask_options`, its title and explicit options create the same `select` card; the selected value or cancellation is returned as the tool result. It deliberately does not redesign the shell or introduce browser-persisted preferences.
