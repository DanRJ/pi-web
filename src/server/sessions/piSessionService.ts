...
export class PiSessionService implements SessionRouteService {
    ...
    return this.serializeLifecycleMutations(refs, async () => {
      ...
      for (const input of plan.archiveInputs) {
        ...
        await this.closeActive(input.sessionId, {
          waitForPendingOpen: false,
          notificationPolicy: { kind: "clear", reason: "archive" },
        });
      ...
      for (const plannedRecord of plan.deleteRecords) {
        ...
        await this.closeActive(record.sessionId, {
          waitForPendingOpen: false,
          notificationPolicy: { kind: "clear", reason: "delete" },
        });
  ...
  async restore(ref: PiSessionLookup): Promise<void> {
    ...
    await this.serializeLifecycleMutations([{ id: archived.sessionId, cwd: archived.cwd }], async () => {
      ...
      await this.closeActive(archived.sessionId, {
        waitForPendingOpen: false,
        notificationPolicy: { kind: "clear", reason: "restore" },
      });
  ...
  async deleteArchived(ref: PiSessionLookup): Promise<void> {
    ...
    await this.serializeLifecycleMutations([{ id: record.sessionId, cwd: record.cwd }], async () => {
      ...
      await this.closeActive(current.sessionId, {
        waitForPendingOpen: false,
        notificationPolicy: { kind: "clear", reason: "delete" },
      });
  ...
  async reload(ref: PiSessionLookup): Promise<void> {
    ...
    try {
      await this.closeActive(sessionId, {
        notificationPolicy: priorGeneration === undefined ? CLEAR_RUNTIME_NOTIFICATIONS : DEFER_RUNTIME_NOTIFICATIONS,
      });
  ...
  private async closeActive(
    sessionId: string,
    options: { waitForPendingOpen?: boolean; notificationPolicy?: NotificationClosePolicy } = {},
  ): Promise<void> {
    ...
    const active = this.active.get(sessionId);
    const notificationPolicy = options.notificationPolicy ?? CLEAR_RUNTIME_NOTIFICATIONS;
  ...
  private async getActive(ref: PiSessionLookup, options: Pick<CreateSessionRuntimeOptions, "notificationGeneration"> = {}): Promise<ActiveSession<PiSessionRuntime>> {
    ...
    return this.openExistingSession(match.id, match.cwd, () => this.openCurrentSessionManager(match.id, match.cwd), options);
  }
  ...
  private openExistingSession(
    ...
    const pending: PendingSessionOpen = {
      ...
      promise: this.serializeLifecycleMutations([{ id: sessionId, cwd }], async () => {
        ...
        return this.create(await openSessionManager(), cwd, options);
      }),
    };
  ...
  private sessionUiContext(
    ...
    const baseUiContext = this.extensionUi.createUiContext(session.sessionId);
    const notify: ExtensionUIContext["notify"] = (message, type) => {
      baseUiContext.notify(message, type);
    ...
    // Keep the browser dialog broker intact while wrapping its notification
    // channel with daemon-owned, bounded inbox state.
